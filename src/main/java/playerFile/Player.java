 ...
 */
enum FactoryRequestType {
    HELP, INCREASE, ATTACK
}
        ...
        if (game.me.myFactories.size() > (game.gameFactories.size() / 2)) {
            ...
            if (toBeIncreased != null) {
                if (toBeIncreased.cyborgsCountMoreOrEqual(10)) {
                    ...
                    requests.add(new Request(toBeIncreased, 10 - toBeIncreased.cyborgsCount, FactoryRequestType.INCREASE));
                }
            }
        }
    }
    ...
    private void bombAction(List<Action> actions) {
        if (this.bombSize > 0 && this.myFactories.size() > 0) {
            ...
            for (Factory opponentFactory : this.opponentFactories) {
                if (opponentFactory.productionSize == 3) {
                    no3 = false;
                    if (opponentFactory.cyborgsCount >= 10 && !opponentFactory.isUnderAttackByBomb()) {
            ...
            if (no3) {
                for (Factory opponentFactory : this.opponentFactories) {
                    if (opponentFactory.productionSize == 2) {
                        no2 = false;
                        if (opponentFactory.cyborgsCount >= 5 && !opponentFactory.isUnderAttackByBomb()) {
                            actions.add(new BombAction(opponentFactory.nearFactory(OwnerState.ME), opponentFactory));
                            this.bombSize--;
                            break;
                        }
            ...
            if (no2) {
                for (Factory opponentFactory : this.opponentFactories) {
                    if (opponentFactory.productionSize == 1) {
                        if (opponentFactory.cyborgsCount >= 5 && !opponentFactory.isUnderAttackByBomb()) {
                            actions.add(new BombAction(opponentFactory.nearFactory(OwnerState.ME), opponentFactory));
                            this.bombSize--;
                            break;
    ...
    public void processing() {
        ...
        /*if (currentTarget == null
            ...
            if (currentTarget != null) {
                ...
                //System.err.println("currentAttackCentre : " + currentAttackCentre.id());
            }
...
class Factory extends Entity {
    ...
    public List<Action> attackActions(Factory currentTarget) {
        ...
        for (Factory factory : this.sortedByDistance) {
            if (!factory.equals(currentTarget) && factory.productionSize > 0) {
